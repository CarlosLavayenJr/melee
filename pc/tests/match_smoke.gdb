...
# it. The button presses come from pc/src/pc_input_script.c (PC_INPUT_SCRIPT=vs)
# rather than from the debugger: a breakpoint on the pad read fires sixty times
# a second, and over the minutes this route takes that was slow enough that the
# test never finished. No scene is overridden and no state is forced -- every
# transition happens because the game's menu code acted on a PADStatus.
#
...
continue
bt 12
printf "in_match=%d match_frames=%d\n", $in_match, $match_frames
if $match_frames < 400
