                         ...
                         unsigned char* addr, unsigned int length)
{
    ...
    /* .dat and .usd -- the same HSD archive format under two extensions, and
       between them the great majority of the disc's content -- are NOT
       converted here. A read is not the right moment: large archives arrive
       in several reads, and devcom.c stages them through relay buffers and
       ARAM rather than into their final memory, so nothing at this layer sees
       a whole archive at the address it will be used from. pc_hsd_archive.c
       converts them where a consumer parses them instead, and says why. */
    return 0;
}
