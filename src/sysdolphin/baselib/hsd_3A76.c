...
s32 HSD_SisLib_803A7F0C(HSD_Text* text, s32 flags)
{
    ...
    while (pos >= 0) {
        ...
        case 1:
            ...
            if (target_type == 1) {
                text->x78.x =
                    (f32) sis_read_s16(text->string_buffer + pos) / 256.0F;
                text->x78.y =
                    (f32) sis_read_s16(text->string_buffer + pos + 2) / 256.0F;
        ...
        case 3:
            ...
            if (target_type == 3) {
                text->x80.x =
                    (f32) sis_read_u16(text->string_buffer + pos) / 256.0F;
                text->x80.y =
                    (f32) sis_read_u16(text->string_buffer + pos + 2) / 256.0F;
                if (flag_hi == entry_flags) {
                    remove_size = 5;
                }
                goto done;
        ...
        case 5:
            pos -= 4;
            if (target_type == 5) {
                result = sis_read_saved_pointer(text->string_buffer + pos);
                if (flag_hi == entry_flags) {
                    remove_size = 5;
                }
                goto done;
...
loop_3:
    ...
    case 14:
        ...
        text->x80.x = (f32) sis_read_u16((u8*) cursor + 1) / 256.0F;
        scale_val = sis_read_u16((u8*) cursor + 3);
        cursor = (u8*) cursor + 4;
    ...
    case 10:
        if ((text->alloc_data == NULL) || (kern_enabled == 0)) {
            ...
            text->x78.x = (f32) sis_read_s16((u8*) cursor + 1) / 256.0F;
        }
        cursor = (u8*) cursor + 4;
        goto block_33;
    case 11:
    ...
    default:
        if (opcode >= 0x20U) {
            *out_width += text->x80.x * (32.0F + text->x78.x);
            if (kern_enabled != 0) {
                glyph_code = sis_read_u16(cursor);
                    ...
                    switch (*sis_cursor) {
                        ...
                        case 5:
                            if (skip_count != 0U) {
                                ...
                                text->x98 = (u32) (text->x98 + 1);
                                text->x94 = sis_read_u16(sis_cursor + 1);
                                text->x60 = (void *) (sis_cursor + 3);
                            }
                            sis_cursor += 2;
                            break;
                        case 6:
                            line_delay = sis_read_u16(sis_cursor + 1);
                            char_delay = sis_read_u16(sis_cursor + 3);
                            sis_cursor += 4;
                            break;
                        case 7:
                            line_started = 1U;
                            HSD_SisLib_803A8134((void*) (sis_cursor + 5), text, &line_width_out, &line_height_out);
                            x_origin = (f32) sis_read_s16(sis_cursor + 1);
                            ...
                            y_offset = sis_read_s16(sis_cursor + 3);
                            sis_cursor += 4;
                        ...
                        case 10:
                            if (((u32) text->alloc_data == 0U) || (saved_kerning == 0)) {
                                ...
                                text->x78.x = (f32) sis_read_s16(sis_cursor + 1) / 256.0F;
                                text->x78.y = (f32) sis_read_s16(sis_cursor + 3) / 256.0F;
                            }
                            sis_cursor += 4;
                            break;
                        case 11:
                        ...
                        case 14:
                            ...
                            text->x80.x = (f32) sis_read_u16(sis_cursor + 1) / 256.0F;
                            text->x80.y = (f32) sis_read_u16(sis_cursor + 3) / 256.0F;
                            sis_cursor += 4;
                            break;
                        case 15:
                        ...
                        default:
                            if (opcode >= 0x20U) {
                                ...
                                glyph_idx = sis_read_u16(sis_cursor);
